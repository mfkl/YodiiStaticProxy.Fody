...
namespace YodiiStaticProxy.Fody.Service
{
    ...
    public class SimpleServiceHostConfiguration : IServiceHostConfiguration, ISimpleServiceHostConfiguration
    {
        ...
        {
            foreach (var m in ReflectionHelper.GetFlattenMethods(type).Where(m => m.Name == methodName))
            {
                _methods[m] = option;
            }
        }
        ...
        {
            foreach (var m in ReflectionHelper.GetFlattenMethods(type).Where(m => !m.IsSpecialName))
            {
                _methods[m] = option;
            }
        }
        ...
        {
            foreach (var p in ReflectionHelper.GetFlattenProperties(type)) SetConfiguration(p, option);
        }
        ...
        {
            foreach (var e in ReflectionHelper.GetFlattenEvents(type))
            {
                _events[e] = option;
            }
        }
